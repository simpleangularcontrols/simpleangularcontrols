...
describe('SacDateSelectorComponent', () => {
    it('should show component', () => {
        cy.mount(
            ...
            {
                imports: [FormsModule, SacFormDirective, SacDateSelectorComponent, SACBootstrap3LayoutModule],
                componentProperties: {},
            }
        );
    ...
    it('should show component only with date', () => {
        ...
        cy.contains('th.text-center', `${monthWithoutLeadingZero}/${year}`).should('exist');

        cy.get('td.day-current').should('have.text', dayWithoutLeadingZero);
    });
    ...
    it('should show component only with time', () => {
        ...
        cy.contains('th.text-center', `${monthWithoutLeadingZero}/${year}`).should('not.exist');
    });
    ...
    it('displays initial month/year when initialvalue is set', () => {
        ...
        cy.get('sac-dateselector')
            .find('.calendar-selector table th.text-center')
            .first()
            .should('contain', `${initial.getMonth() + 1}/${initial.getFullYear()}`);

        cy.get('td.day-selected').should('have.text', '15');
    });
    ...
    it('navigates months with next and prev controls', () => {
        ...
        cy.get('sac-dateselector').find('.calendar-selector th.text-right').click();
        cy.get('sac-dateselector')
            .find('.calendar-selector table th.text-center')
            .first()
        ...
        cy.get('sac-dateselector').find('.calendar-selector th.text-left').click();
        cy.get('sac-dateselector')
            .find('.calendar-selector table th.text-center')
            .first()
            .should('contain', `${initial.getMonth() + 1}/${initial.getFullYear()}`);
    ...
    it('should set correct date with selected date', () => {
        ...
        cy.contains('.calendar-selector tbody td.text-center', '10').click();
    ...
    it('should switch to december to january when click back', () => {
        ...
        cy.get('sac-dateselector').find('.calendar-selector table th.text-center').first().should('contain', '1/2022');

        // click prev (left control)
        cy.get('sac-dateselector').find('.calendar-selector th.text-left').click();
        cy.get('sac-dateselector').find('.calendar-selector table th.text-center').first().should('contain', '12/2021');
    });
    ...
    it('should switch to january to december when click next', () => {
        ...
        cy.get('sac-dateselector').find('.calendar-selector table th.text-center').first().should('contain', '12/2022');

        // click prev (right control)
        cy.get('sac-dateselector').find('.calendar-selector th.text-right').click();
        cy.get('sac-dateselector').find('.calendar-selector table th.text-center').first().should('contain', '1/2023');
    });
