...
describe('SacDateComponent', () => {
    ...
    it('should reset date via selector with reactive forms', () => {
        const form = new FormGroup({
            date: new FormControl(new Date(2025, 12 - 1, 5)), // Month is Index and not Month Value
        });
    ...
    it('should set date via selector', () => {
        ...
        cy.get('.calendar-selector div:not(.day-disabled)')
    ...
    it('should set date via selector when item is at bottom', () => {
        ...
        cy.get('.calendar-selector').parents('.popover').should('have.class', 'bs-popover-auto');
    ...
    it('cancel via selector should not change value', () => {
        ...
        cy.get('button').click();
        cy.contains('.calendar-selector div:not(.day-disabled)', dayWithoutLeadingZero).click();
        cy.get('#field').click();
